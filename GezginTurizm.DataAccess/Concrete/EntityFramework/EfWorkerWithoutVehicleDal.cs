...
{
    public class EfWorkerWithoutVehicleDal : EfEntityRepositoryBase<WorkerWithoutVehicle, GezginContext>, IWorkerWithoutVehicleDal
    {
        public void ChangeReadStatus(int id)
        {
            using (GezginContext context = new GezginContext())
            {
                var model = context.WorkerWithoutVehicles.Where(x => x.WorkerId==id).FirstOrDefault();
                if (!model.isRead)
                {
                    model.isRead = true;
                    context.SaveChanges();
        ...
        public int CountUnreadNotification()
        {
            using (GezginContext context = new GezginContext())
            {
                return context.WorkerWithoutVehicles.Count(x => x.isRead == false);
            }
        }
        ...
        public void SendMail()
        {
            ...
            message.Subject = "Yeni İşçi Başvurusu Bildirimi";
            message.Body = DateTime.Now.ToString() + " tarihinde bir yeni işçi başvurusu alınmıştır.Website paneline giderek kontrol edebilirsiniz.";
            client.Send(message);
        }
    }
}
